app.controller('mallVerificationcounters.IndexCrtl', ['$scope', '$http', 'SweetAlert','toaster', function ($scope, $http, SweetAlert,toaster) {
        
        $scope.nwVerification= {};
        $scope.currentPgNo = 1;
        $scope.options = [
            {id: 1, name: 'Active'},
            {id: 0, name: 'Deactive'}
        ...
        $scope.getData = function () {
            ...
            $http.post('/mall-verificationcounters.json').success(function (d) {
                $scope.verificationcounters = d.mallVerificationcounters;
                $scope.pager = d.pager;
        ...
        $scope.changeStatus = function (v, id) {
            var res = $http.post("/mall-verificationcounters/change-status.json", {id: id, status: v});
        ...
        $scope.form = {
            submit: function (form) {
                ...
                if (form.$invalid) {
                    ...
                    $scope.isLoading = true;
                    $http.post("/mall-verificationcounters/add.json", $scope.nwVerification).success(function (d) {
                        if (d.result.error == 0) {
